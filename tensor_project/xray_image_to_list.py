...
class ImageToCSV:
    ...
    def _setLabel(self): #label.csv 에서 (p/n) 라벨 정보를 가져와서 리스트에 저장
        file = open(self.__LABEL_PATH, 'r')
        reader = csv.reader(file, delimiter=',')
        list = ['dummy'] # 리스트가 0번부터 시작하는데 1~10.png라서 숫자맞춰주려고 0번째에 'dummy'를 그냥 넣어준거임
        for r in reader:
            list.append(r[0])
        return list
    ...
    def _image_to_thumbnail(self):
        ...
        '''
        size = (32, 32) #정방크기가 아닌 경우 여백에 흰색 바탕
        for file in [filename for filename in os.listdir(self.__IMAGE_PATH) if
            ...
            try:
                ...
                new_img = Image.new("RGB", (32, 32), "white") #32*32 흰색 ㅇ ㅣ미지
                im = Image.open(self.__IMAGE_PATH + str(file))
                im.thumbnail(size, Image.ANTIALIAS)
                load_img = im.load()
                load_newimg = new_img.load()
                i_offset = (32 - im.size[0]) / 2 #중앙에 맞춰주려고 위치 조정
                j_offset = (32 - im.size[1]) / 2

                for i in range(0, im.size[0]): #사진 그려줌
    ...
    def _rgb2gray(self, rgb, type):
        ...
        if type == 'png':
            gray = 255 * (0.2126 * r + 0.7152 * g + 0.0722 * b) #0~1 사이의 소수점으로 되어있어서 0~255사이의 값으로 바꿔주려고 *255
        elif type == 'jpg':
            gray = 0.2126 * r + 0.7152 * g + 0.0722 * b

        return np.array(gray).astype('int32') #rgb가 소수점을 안써서 int값으로 씀
    ...
    def img_to_csv(self):
        # 이미지를 썸네일로 바꾸어 저장
        print('img to thumbnail start.')
        self._image_to_thumbnail()
        print('img to thumbnail end.')
