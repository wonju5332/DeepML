...
class Model:
    ...
    def _build_net(self):
        # scopename = 'self.name'
        with tf.variable_scope(self.name):    #tf.get_variable()에 전달 된 이름의 네임 스페이스를 관리합니다.
            ...
            self.product = tf.matmul(self.L6, self.W7) + self.b7

        self.cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=self.product, labels=self.Y)) + 0.01*tf.reduce_sum(tf.square(self.W7))
        self.optimizer = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.cost)
        self.accuracy = tf.reduce_mean(tf.cast(tf.equal(tf.arg_max(self.product, 1), tf.arg_max(self.Y, 1)), dtype=tf.float32))

    def predict(self, x_test):
        return self.sess.run(self.product, feed_dict={self.X: x_test, self.training: False})
...
for epoch in range(training_epochs):  #20   , 총 에폭 20을 회전한다.
    avg_cost_list = np.zeros(len(models))  #[0,0]
    total_batch = int(mnist.train.num_examples / batch_size)   # 55,000/100  --> 550

    for i in range(total_batch):
        batch_xs, batch_ys = mnist.train.next_batch(batch_size)   # 원핫인코딩
        # 각각의 모델 훈련
        for idx, m in enumerate(models):  # 0 , 1
            c, a, _ = m.train(batch_xs, batch_ys)   # m.train 은 cost/accuracy 를 뱉어내다보다.  #실제치와 예측치 비교??
