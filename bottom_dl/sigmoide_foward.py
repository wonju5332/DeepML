...
class TwoLayerNet:
    def __init__(self, input_size, hidden_size, output_size, weight_init_std=0.01):
        # 가중치 초기화
        self.params = {}
        f_weight_init_std = (1/np.sqrt(784))
        s_weight_init_std = (1/np.sqrt(50))
        self.params['W1'] = f_weight_init_std * np.random.randn(input_size, hidden_size)
        self.params['b1'] = np.zeros(hidden_size)
        self.params['W2'] = s_weight_init_std * np.random.randn(hidden_size, hidden_size)
        self.params['b2'] = np.zeros(hidden_size)
        self.params['W3'] = s_weight_init_std * np.random.randn(hidden_size, hidden_size)
        self.params['b3'] = np.zeros(hidden_size)
        self.params['W4'] = s_weight_init_std * np.random.randn(hidden_size, hidden_size)
        self.params['b4'] = np.zeros(hidden_size)
        self.params['W5'] = s_weight_init_std * np.random.randn(hidden_size, output_size)
...
network = TwoLayerNet(input_size=784, hidden_size=190, output_size=10)
...
for i in range(iters_num): # 10000
    ...
    # Adam 방법
    # model = Adam()
    # model.update(network.params, grad)
    # Adagrad방법
    # model = AdaGrad()
    # model.update(network.params, grad)
    # for key in network.params.keys():  # 키만 가지고 옴
    #     v[key] = 0.9 * v[key] - learning_rate * grad[key]
